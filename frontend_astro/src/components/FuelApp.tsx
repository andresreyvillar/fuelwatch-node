...
import { Search, Fuel as FuelIcon, Droplets, ChevronDown, Check, Menu, X, Sun, Moon, Clock } from 'lucide-react';
...
const FuelApp: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (window.innerWidth >= 800) setIsSidebarOpen(true);
  ...
  const lastUpdate = useMemo(() => {
    const allStations = [...stations, ...pinnedStations];
    if (allStations.length === 0) return null;
    const dates = allStations.map(s => s.fecha_actualizacion).filter(Boolean).map(d => new Date(d).getTime());
  ...
  return (
    <div className='h-[100dvh] w-full bg-gray-50 astro-dark:bg-[#0a0a0a] flex flex-col desk:flex-row transition-colors duration-300 overflow-hidden'>
      ...
      <div className='flex-1 flex flex-col h-[100dvh] overflow-hidden'>
        ...
        <div className='flex-1 overflow-y-auto overscroll-y-contain custom-scrollbar p-4 desk:p-8'><div className='max-w-4xl w-full mx-auto'>
